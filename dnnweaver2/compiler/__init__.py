...
class GraphCompiler(object):
    ...
    def _gemm_compile(self, conv_op, pu_op):
        """
        Compiler for gemm layers
        TODO: replace hard-coded array sizes
        """
        inst_array = []
        inst_array.append(SetupInstruction(16, 16).get_binary())

        self.log.debug('conv op: {}'.format(conv_op.name))
    ...
    def compile(self, graph, acc_obj):
        ...
        for i in range(len(macro_node_array)):
            ...
            # Assuming GEMM, we pad the input rows to number of rows
            # [TODO]: Remove channels from the input, it seems to be irrelevant
            ic = macro_node.sys_array_op.data.shape[-3]
        ...
        for i in range(len(macro_node_array)):
            ...
            # inst_array = self._gemm_compile(conv_op=macro_node.sys_array_op, pu_op=macro_node.pu_op)
            # print(inst_array)
